...
- **Global scope**: `/home/rkujawa/repos/uiscsi/.claude/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `/home/rkujawa/repos/uiscsi/.claude/notes/`)
...
2. Glob `/home/rkujawa/repos/uiscsi/.claude/notes/*.md` (if directory exists) — global notes
...
Global (/home/rkujawa/repos/uiscsi/.claude/notes/):
...
2. **No `.planning/`**: Falls back to global `/home/rkujawa/repos/uiscsi/.claude/notes/` — works in any directory
